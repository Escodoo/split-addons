...
| `tags` / `tag_ids` | Comma- or semicolon-separated; missing tags are created automatically. |
| `description` | Stored on the lead and posted as a chatter internal note. |
| `priority` | `Low` / `Medium` / `High` / `Very High` (or 0/1/2/3) maps to the priority stars widget. |
| `website` / `Site` | |
| `city` / `Cidade` | |

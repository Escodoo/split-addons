...
This module provides a wizard for importing leads from CSV or XLSX
files with advanced duplicate checking. Before creating the leads in
CRM, it checks existing records in both ``crm.lead`` and
``res.partner`` by:
...
1. Go to **CRM > Pipelines** and open the desired pipeline's
   **Import Leads** action, or use **CRM > Import Sessions** to review
   previous imports.
